...
},showTaskFormButton:function(){if(this.currentShapes&&this.currentShapes[0]&&this.currentShapes[0].properties&&this.currentShapes[0].properties["oryx-tasktype"]&&this.currentShapes[0].properties["oryx-tasktype"]=="User"){this.taskFormButton.prepareToShow()
...
a.each((function(e){var d=new Ext.menu.Item({text:e.title(),icon:e.icon(),disabled:e.id()==c[0].getStencil().id(),disabledClass:ORYX.CONFIG.MORPHITEM_DISABLED,handler:(function(){this.morphShape(c[0],e)
}).bind(this)});
...
Ext.Ajax.request({url:ORYX.PATH+"taskformseditor",method:"POST",success:function(q){try{var s=q.responseText.evalJSON();
parent.designersignalassetadded(s.formid);
parent.designersignalassetupdate(s.formid);
p.close();
p=null
